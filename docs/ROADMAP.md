...
- [ ] **App Store Connect setup** — capability, upload, and metadata automation now pass: `Check App Store Capability` run `25989507219`, tagged `Build iOS` run `25989519652`, and metadata/screenshots sync run `25989760214`. Remaining App Store Connect work is processed-build selection plus manual privacy, pricing/availability, age rating, regulated medical device, and accessibility-label confirmations. See `docs/app-store-pre-submission-runbook.md`.
- [x] **App Store product page assets** — text metadata and App Review notes are stored under `ios/fastlane/metadata`; five iPhone 6.9-inch screenshots are prepared under `ios/fastlane/screenshots/en-US` and were confirmed present by metadata run `25989760214`.
- [x] **TestFlight pipeline verification** — tagged `v1.0.31` workflow run `25989519652` checked out `ba9d896` and uploaded the signed IPA to App Store Connect/TestFlight; build processing still needs to be confirmed in App Store Connect before selecting `1.0.31 (286)` on the public version page.
