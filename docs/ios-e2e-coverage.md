...
`--ui-only` runs `ios_app_test.dart` and then exits with that suite's result. It skips native channel wiring, paired Watch, auth, startup and process-restart suites; it cannot be combined with `--with-auth` or `--paired-watch`. Firebase CLI, Java and AXe are not needed for this mode. CI runs the iPad step only when the manual Build iOS workflow sets `run_ipad=true`. Set `quality_only=true` as well to verify without uploading. Normal tags and PRs skip simulator E2E suites at the user's request because they are unreliable. Manual Build iOS dispatch can opt in with `run_native_e2e=true`; iPad remains independently opt-in. App unit/widget tests, package tests, Swift lifecycle tests, Python helper tests, Firestore emulator rules checks, analysis and static-site checks remain required. Release archives still build the iOS app, Watch app and Live Activity extension. A release with simulator checks disabled does not establish full end-to-end acceptance. When enabled, CI uploads `build/ios-e2e-ipad/` alongside the phone/Watch evidence.
